...
- **로직 2 (필터링):**
  ...
  - B열(진행상태) 값이 `'취소종료'` 또는 `'중단종료'`인 행은 완전히 제외
...
- **로직 3 (기본 데이터 매핑):**
  ...
  - 업무시스템: E열 또는 F열
  - 업무시스템2: W열
  - 진행상태: B열 값 → `'종료/요청처리확인/요청처리승인'` → `완료` / `'SR사전검토/SR사전검토승인/요청접수및분류'` → `대기` / 그 외 → `진행중`

> **[데이터 정합성 원칙]** 진행상태(B열) 값을 상수와 비교할 때, 앞뒤 공백은 물론 문자열 내부의 모든 공백을 제거한 후 판단한다.
> (구현: Pandas 시리즈는 `.str.replace(r'\s+', '', regex=True)`, 단일 값은 `re.sub(r'\s+', '', val)`)
