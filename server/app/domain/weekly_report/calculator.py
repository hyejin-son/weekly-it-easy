...
class WeeklyReportCalculator(
    ...
    def _build_batch_prompt(self, input_data: list[dict]) -> str:
        ...
        return (
            ...
            "【절대 금지 사항 — Over-summarization 방지】\n"
            "- 이 작업은 '요약(Summarize)'이 아니라 '재작성(Rewrite)'이다. "
            "원본의 내용 길이와 관계없이 모든 정보를 빠짐없이 보존해야 한다.\n"
            "- 원본 텍스트에 포함된 모든 정보(배경, 원인, 수치, 메뉴명, 시스템 연동, 처리 경위 등)를 100% 보존할 것.\n"
            "- 내용을 임의로 축약하거나 생략하지 말 것. "
            "단순히 문체를 '비즈니스 개조식'으로 다듬고 가독성만 높이는 재작성(Rewriting)을 수행하라.\n"
            "- 원본에 있는 수치(예: 0.5%, 100건)나 구체적 항목(예: 메뉴명, 시스템명, 오류 원인)을 "
            "하나라도 빠뜨리면 실패한 결과물이다.\n\n"
            "【핵심 원칙】\n"
            ...
            "【필드별 작성 규칙】\n"
            "- [제목]: 업무 핵심을 담아 1줄로 작성.\n"
            "- [개요]: 비즈니스 배경·목적·기대효과 중심으로 작성. "
            "원본의 구체적 수치, 메뉴명, 시스템명을 반드시 유지. "
            "길이는 원본 정보를 모두 포함하는 데 필요한 만큼 자연스럽게 결정.\n"
            "- [내용]: 처리내용이 있으면 원본의 모든 처리 항목과 수치를 빠짐없이 포함하여 구조화하여 작성. "
